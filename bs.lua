...
-- Identify our executor
local executor = syn and "Synapse" or identifyexecutor()
...
local Themes = {
	default = {
		...
		Accent = Color3.fromRGB(115, 143, 66)
	},
...
local MenuParameters = {
	...
	CheatName = 'Bloxsense',
	UserType = env.login and env.login.username or 'FLY HIGH INVADED REST IN PEACE',
...
do
	...
	UILibrary:CreateButton({Name = "Ragebot", Tab = "Rage", Section = "Aimbot", KeyBind = "None", Tooltip = "Master switch for aimbot"})
	...
	UILibrary:CreateButton({Name = "Hitlog", Tab = "Rage", Section = "Aimbot", Tooltip = "Sends a notification everytime you shoot"})
	...
	UILibrary:CreateDropdown({Name = "Double Tap", Tab = "Rage", Section = "Aimbot", Values = {"Off", "Fast", "Faster", "Fastest", 'Dump'}, Tooltip = "Controls if the aimbot may shoot two bullets in quick succession"})
	...
	UILibrary:CreateButton({Name = "Float", Tab = "Rage", Section = "Anti Aim", Tooltip = "Positions your body higher to make you float."})
	UILibrary:CreateButton({Name = "Invisible", Tab = "Rage", Section = "Anti Aim", Tooltip = "Lowers your body to be completely invisible"})
	...
	UILibrary:CreateButton({Name = "Remove Animations", Tab = "Rage", Section = "Anti Aim", Tooltip = "Overrides all animations with empty ones."})
	...
	UILibrary:CreateSlider({Name = "Third Person Distance", Tab = "Visuals", Section = "Camera", MinimumNumber = 50, MaximumNumber = 250, DefaultValue = 90})
	...
	UILibrary:CreateDropdown({Name = "Bullet Tracer Texture", Tab = "Visuals", Section = "Bullets", Values = {'Plain Thin', 'Plain', 'Electro Beam', 'Electro Beam 2', 'Alternate Beam', 'Lighting Beam 1', 'Lighting Beam 2', 'Energy Beam'}})
	...
	--UILibrary:CreateDropdown({Name = "Bullet Tracer Origin", Tab = "Visuals", Section = "Bullets", Values = {"Automatic", "Barrel", "Camera", 'HumanoidRootPart'}})
	...
	UILibrary:CreateSlider({Name = "Minimum Ping", Tab = "Misc", Section = "Exploits", MinimumNumber = 1, MaximumNumber = 3000, DefaultValue = 60, Suffix = "ms"})
	UILibrary:CreateSlider({Name = "Maximum Ping", Tab = "Misc", Section = "Exploits", MinimumNumber = 1, MaximumNumber = 3000, DefaultValue = 250, Suffix = "ms"})
	UILibrary:CreateButton({Name = "Ping Troll", Tab = "Misc", Section = "Exploits"})
	UILibrary:CreateDropdown({Name = "Ping Troll Mode", Tab = "Misc", Section = "Exploits", Values = {'-69', '0', 'Freeze', 'Infinite'}})
	...
	UILibrary:CreateButton({Name = "Menu Accent", Tab = "Settings", Section = "Menu Settings", Colors = {Color3.fromRGB(115, 143, 66)}})
	...
	UILibrary:CreateButton({Name = "Custom Identifiers", Tab = "Settings", Section = "Menu Settings"})
	UILibrary:CreateTextBox({Name = "Custom Menu Name Text", Tab = "Settings", Section = "Menu Settings", Default = 'Bloxsense'})
	UILibrary:CreateTextBox({Name = "Custom Login Username", Tab = "Settings", Section = "Menu Settings", Default = 'Developer'})
	...
	if not isfolder("bloxsense/bloxsense_luas") then
		makefolder("bloxsense/bloxsense_luas")
	end
	...
	UILibrary:CreateDropdown({
    	Name = "Configs", Tab = "Settings", Section = "Configurations", 
    	Values = getconfigs(), 
    	Callback = function(selection) 
        	Menu["Settings"]["Configurations"]["Config Text"]["Value"] = selection 
    	end
	})
	UILibrary:CreateTap({
    	Name = "Load", Tab = "Settings", Section = "Configurations", Confirmation = true, 
    	Callback = function() 
        	local old = Menu["Settings"]["Configurations"]["Config Text"]["Value"] 
        	UILibrary:LoadConfiguration(readfile('bloxsense/bloxsense_configs/' .. Menu["Settings"]["Configurations"]["Config Text"]["Value"]))
        	Library.UI:EventLog("Loaded " .. Menu["Settings"]["Configurations"]["Config Text"]["Value"], 5)
        	Menu["Settings"]["Configurations"]["Config Text"]["Value"] = old 
    	end
	})
	UILibrary:CreateTap({
    	Name = "Reload", Tab = "Settings", Section = "Configurations", Confirmation = true, 
    	Callback = function() 
        	Menu["Settings"]["Configurations"]["Configs"].UpdateValues(getconfigs()) 
    	end
	})
	UILibrary:CreateTap({
    	Name = "Save", Tab = "Settings", Section = "Configurations", Confirmation = true, 
    	Callback = function() 
        	local old = Menu["Settings"]["Configurations"]["Config Text"]["Value"]
        	writefile('bloxsense/bloxsense_configs/' .. Menu["Settings"]["Configurations"]["Config Text"]["Value"], UILibrary:SaveConfiguration())
        	Library.UI:EventLog("Saved " .. Menu["Settings"]["Configurations"]["Config Text"]["Value"], 5)
        	Menu["Settings"]["Configurations"]["Config Text"]["Value"] = old 
    	end
	})
	UILibrary:CreateTap({
    	Name = "Delete", Tab = "Settings", Section = "Configurations", Confirmation = true, 
    	Callback = function() 
        	local old = Menu["Settings"]["Configurations"]["Config Text"]["Value"]
        	delfile('bloxsense/bloxsense_configs/' .. Menu["Settings"]["Configurations"]["Config Text"]["Value"])
        	Library.UI:EventLog("Deleted " .. Menu["Settings"]["Configurations"]["Config Text"]["Value"], 5)
        	Menu["Settings"]["Configurations"]["Configs"].UpdateValues(getconfigs())
        	Menu["Settings"]["Configurations"]["Config Text"]["Value"] = old 
    	end
	})
...
do --ANCHOR Visuals
	...
	do --esp and chams
		...
		function visuals.initializeDrawingObjects(onScreen, offScreen, general)
			...
			onScreen.nameTag            = {
				object = visuals.createDrawing("Text", {['OutlineColor'] = Color3.fromRGB(0, 0, 0), ['Outline'] = true}, visuals.defaultProp.nameTag),
				originalTransparency = visuals.defaultProp.nameTag.Transparency
			}
			onScreen.heldWeapon         = {
				object = visuals.createDrawing("Text", {['OutlineColor'] = Color3.fromRGB(0, 0, 0), ['Outline'] = true}, visuals.defaultProp.heldWeapon),
			...
			onScreen.healthNumber       = {
				object = visuals.createDrawing("Text", {['OutlineColor'] = Color3.fromRGB(0, 0, 0), ['Outline'] = true}, visuals.defaultProp.healthNumber),
				originalTransparency = visuals.defaultProp.healthNumber.Transparency
			}
			onScreen.distanceTag        = {
				object = visuals.createDrawing("Text", {['OutlineColor'] = Color3.fromRGB(0, 0, 0), ['Outline'] = true}, visuals.defaultProp.distanceTag),
				originalTransparency = visuals.defaultProp.distanceTag.Transparency
			}
			onScreen.armorTag        = {
				object = visuals.createDrawing("Text", {['OutlineColor'] = Color3.fromRGB(0, 0, 0), ['Outline'] = true}, visuals.defaultProp.flags),
		...
		function visuals.applyEsp(player)
			...
			function this.renderOnScreen(info)
				...
				if Menu["ESP"][section]["Held Weapon"]["Toggle"]["Enabled"] then
					...
					heldWeapon.OutlineColor = Color3.fromRGB(0, 0, 0)
				...
				if Menu["ESP"][section]["Distance"]["Toggle"]["Enabled"] then
					distanceTag.Text = mathModule.truncateNumber((this.drawingObjects.lastData.headCf.p - camera.CFrame.p).Magnitude / 3.571428571428571, 1) .. (textCase == "lowercase" and " m" or textCase == "UPPERCASE" and " m" or " m")
				...
				if Menu["ESP"][section]["Name"]["Toggle"]["Enabled"] then
					...
					nameTag.Position = boundingRect.Min + newVector2(math.floor(boundingRect.Width / 2), -2 - 14)
					nameTag.Center = true
	...
	do --ANCHOR Bullet Tracer
		...
		function visuals.bulletTracer(origin, target)
			...
			if origin == nil then
				...
				if Menu["Visuals"]["Camera"]["Third Person"]["Toggle"]["Enabled"] and Menu["Visuals"]["Camera"]["Third Person"]["Bind"]["Active"] then
					cameraCf = cameraCf * newCframe(0, 2, -Menu["Visuals"]["Camera"]["Third Person Distance"].Value/10)
				end
				if m == "Automatic" then
				...
				elseif m == "Camera" then
					origin = cameraCf.p
				elseif m == "HumanoidRootPart" then
			...
			local txture = Menu["Visuals"]["Bullets"]["Bullet Tracer Texture"]["Value"]
			object.beam.Texture = "rbxassetid://446111271"
			object.beam.TextureLength = 12
			object.beam.TextureSpeed = 1
			...
			if txture == 'Electro Beam' then
				object.beam.Texture = "rbxassetid://446111271"
			elseif txture == 'Plain' then
				object.beam.Texture = "rbxassetid://78474503458021"
			elseif txture == 'Plain Thin' then
				object.beam.Texture = "rbxassetid://126112805316900"
			elseif txture == 'Electro Beam 2' then
				object.beam.Texture = "rbxassetid://7071778278"
			elseif txture == 'Alternate Beam' then
				object.beam.Texture = "rbxassetid://128372145766358"
			elseif txture == 'Lighting Beam 1' then
				object.beam.Texture = "rbxassetid://7151778302"
			elseif txture == 'Lighting Beam 2' then
				object.beam.Texture = "rbxassetid://7151842823"
			elseif txture == 'Energy Beam' then
				object.beam.Texture = "rbxassetid://6091341618"
			end
		...
		function visuals.updateTracerProperties()
			for i,v in next, visuals.tracerParent:GetChildren() do
				if v.ClassName == "Beam" then
					...
					if txture == 'Electro Beam' then
						v.Texture = "rbxassetid://446111271"
					elseif txture == 'Lighting Beam 1' then
						v.Texture = "rbxassetid://7151778302"
					elseif txture == 'Lighting Beam 2' then
						v.Texture = "rbxassetid://7151842823"
					elseif txture == 'Energy Beam' then
						v.Texture = "rbxassetid://6091341618"
					end
				end
			end
		end
...
do --ANCHOR Ragebot
	...
	runService:BindToRenderStep("cameraFix", Enum.RenderPriority.Camera.Value + 1, function()
		if Menu["Visuals"]["Camera"]["Third Person"]["Toggle"]["Enabled"] and Menu["Visuals"]["Camera"]["Third Person"]["Bind"]["Active"] then
			...
			camera.CFrame = camera.CFrame * CFrame.new(0, 1.25, (pos - camera.CFrame.p).Magnitude - (wall and 0.15 or 0))
    ...
    local fakeReplicationUpdate = function(uptime, delta)
		...
		if Menu["Rage"]["Anti Aim"]["Hide in Floor"]["Toggle"]["Enabled"] then
			delayedcf = delayedcf - newVector3(0, 5.25, 0)
		end

		if Menu["Rage"]["Anti Aim"]["Float"]["Toggle"]["Enabled"] then
			delayedcf = delayedcf - newVector3(0, -7, 0)
		end

		if Menu["Rage"]["Anti Aim"]["Invisible"]["Toggle"]["Enabled"] then
			delayedcf = delayedcf - newVector3(0, -9e9, 0)
		end
	...
	function ragebot.fire(Parameters) -- everything that takes place once someone CAN be shot
		...
		if Menu["Rage"]["Aimbot"]["Auto Shoot"]["Toggle"]["Enabled"] then
			...
			if Menu["Rage"]["Aimbot"]["Hitlog"]["Toggle"]["Enabled"] then
				Library.UI:EventLog(string.format('Shot %s for %s in [%s] with (%s health remaining)', tostring(ragebot.currenttarget.player), tostring(math.floor(Parameters.damageInflicted + 0.5)), tostring(ragebot.currenttarget.instance), tostring(math.clamp(math.floor(Parameters.instance.Parent.Humanoid.Health - (Parameters.damageInflicted * client.fgun.Bullets.Value) + 0.5), 0, 100))), 5)
			end
	...
	function ragebot.think() -- Invaded#5143 (girl)
		if not (localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") and Menu["Rage"]["Aimbot"]["Ragebot"]["Toggle"]["Enabled"] and Menu["Rage"]["Aimbot"]["Ragebot"]["Bind"]["Active"]) or not client.fgun or client.fgun == "none" or Menu["Misc"]["Extra"]["Kill All"]["Toggle"]["Enabled"] then
		...
		local result
		if gun:FindFirstChild("Melee") and Menu["Rage"]["Aimbot"]["Knife Bot"]["Toggle"]["Enabled"] then -- okay so we're holding our knife so we need to use our knifebot
		...
		if Menu["Rage"]["Aimbot"]["Auto Shoot"]["Toggle"]["Enabled"] then
			...
			local shifting = Menu["Rage"]["Aimbot"]["Double Tap"]["Value"] == "Fast" and 10/64 or Menu["Rage"]["Aimbot"]["Double Tap"]["Value"] == "Faster" and 12/64 or Menu["Rage"]["Aimbot"]["Double Tap"]["Value"] == "Fastest" and 14/64 or enu["Rage"]["Aimbot"]["Double Tap"]["Value"] == "Dump" and 32/64
			...
			if Menu["Visuals"]["Camera"]["Hit Marker"]["Toggle"]["Enabled"] then
				...
				if Menu["Visuals"]["Camera"]["Hit Marker Type"].Value == "2D" then
					...
					task.spawn(function()
						...
						local damagedealt = tostring(dmgdealt)
						local text2 = visuals.createDrawing("Text", {
							Color = Color3.new(1, 1, 1),
							OutlineColor = Color3.new(0, 0, 0),
							Visible = true,
							Text = 'dealt',
						...
						local text = visuals.createDrawing("Text", {
							...
							OutlineColor = Color3.new(0, 0, 0),
							Visible = true,
							Text = string.format('%s dmg', damagedealt),
							Size = 13,
							Center = true,
							Outline = true,
							Font = 2
						...
						posSignal:Connect(function(pos)
							...
							text2.Position = newVector2(pos.x, pos.y - 33)
						end)
						transSignal:Connect(function(newTrans)
							if type(newTrans) == "number" then
								text.Transparency = newTrans
								text2.Transparency = newTrans
							else
								text:Remove()
								text2:Remove()
								text = nil
	...
	local oldNamecall; oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
		...
		if method == "FireServer" then
			...
			elseif self.Name == "UpdatePing" and Menu["Misc"]["Exploits"]["Ping Spoofer"]["Toggle"]["Enabled"] then
				...
				return oldNamecall(self, table.unpack(args,1,select("#",...)))
			elseif self.Name == "UpdatePing" and Menu["Misc"]["Exploits"]["Ping Troll"]["Toggle"]["Enabled"] then
				if Menu["Misc"]["Exploits"]["Ping Troll Mode"]["Value"] == '-69' then
					args[1] = -69/1000
				elseif Menu["Misc"]["Exploits"]["Ping Troll Mode"]["Value"] == 'Infinite' then
					args[1] = math.huge/1000
				elseif Menu["Misc"]["Exploits"]["Ping Troll Mode"]["Value"] == '0' then
					args[1] = 0
				elseif Menu["Misc"]["Exploits"]["Ping Troll Mode"]["Value"] == 'Freeze' then
					args[1] = 'Nigger'
				end
				return oldNamecall(self, table.unpack(args,1,select("#",...)))
			...
			elseif self.Name == hitPart.Name and not Menu["Misc"]["Extra"]["Kill All"]["Toggle"]["Enabled"] then
				local x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16,x17,x18,x19,x20,x21,x22,x23,x24 = ...
				...
				if Menu["Misc"]["Weapon Modifications"]["Enabled"]["Toggle"]["Enabled"] then
					x7 = x7 * (Menu["Misc"]["Weapon Modifications"]["Damage Scale"]["Value"]/100)
				end
				...
				if x14 == "r" then
					...
					x14 = nil
					return oldNamecall(self, x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16,x17,x18,x19,x20,x21,x22,x23,x24)
				end
                if Menu["Misc"]["Exploits"]["Shot players become mush"]["Toggle"]["Enabled"] then
                    x1 = x1.Parent.FindFirstChild(x1.Parent, "Head") or x1
                    x7 = 1
                    x3 = "AWP"
                    oldNamecall(self, x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16,x17,x18,x19,x20,x21,x22,x23,x24)
                    x3 = "Multimeter"
                    oldNamecall(self, x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16,x17,x18,x19,x20,x21,x22,x23,x24)
                    return
                end
		...
		elseif method == "LoadAnimation" then
			if (Menu["Misc"]["Movement"]["Automatic Jump"]["Toggle"]["Enabled"] and (Menu["Misc"]["Movement"]["Speed"]["Toggle"]["Enabled"] and Menu["Misc"]["Movement"]["Speed"]["Bind"]["Active"])) or Menu["Rage"]["Anti Aim"]["Slide Walk"]["Toggle"]["Enabled"] then
				if string.match(args[1].Name, "Jump") or (string.match(args[1].Name, "Run") or string.match(args[1].Name, "Walk") or string.match(args[1].Name, "Crouch")) then
					args[1] = ragebot.fakeanimation
					return oldNamecall(self, args[1])
				end
			end
			if Menu["Rage"]["Anti Aim"]["Remove Animations"]["Toggle"]["Enabled"] then
				args[1] = ragebot.fakeanimation
				return oldNamecall(self, args[1])
			end
...
do
	...
	function menusettings.updatecheattext()
		if Menu["Settings"]["Menu Settings"]["Custom Identifiers"]["Toggle"]["Enabled"] == true or Menu["Settings"]["Menu Settings"]["Custom Username"]["Toggle"]["Enabled"] == true then
			Library.UI.CheatNameText.Text = Menu["Settings"]["Menu Settings"]["Custom Menu Name Text"]["Value"]
			Library.UI.Setwatermarkcheatname(Menu["Settings"]["Menu Settings"]["Custom Menu Name Text"]["Value"], Menu["Settings"]["Menu Settings"]["Custom Login Username"]["Value"])
		else
			Library.UI.Setwatermarkcheatname(MenuParameters.CheatName, Parameters.UserType)
			Library.UI.CheatNameText.Text = MenuParameters.CheatName
		end
	end
	Menu["Settings"]["Menu Settings"]["Custom Identifiers"]["Toggle"].Changed:Connect(menusettings.updatecheattext)
	Menu["Settings"]["Menu Settings"]["Custom Menu Name Text"].Changed:Connect(menusettings.updatecheattext)
    Menu["Settings"]["Menu Settings"]["Custom Login Username"].Changed:Connect(menusettings.updatecheattext)
...
LogService.MessageOut:Connect(function(message, messageType)
	if messageType == Enum.MessageType.MessageError and message:match("kicked") then
		...
		if message == 'Server Kick Message: You have been votekicked' then
		    Library.UI:EventLog('You have been votekicked', 999)
	    else
		    Library.UI:EventLog("You were kicked from the game for: " .. message, 999)
	    end
	end
end)
...
if not (executor == "ScriptWare" and platform == "Mac") then
	...
	for i, v in next, ({"Network Send: ", "Network Recieve: ", "FPS: ", "Tick: ", "Memory Usage: ", "Status: ", "Connected"}) do
		local Txt = visuals.createDrawing("Text", {["Size"] = 13, ["Center"] = false, ["Outline"] = true, ["Font"] = Drawing.Fonts.Plex, ["Color"] = Color3.fromRGB(255, 255, 255), ['OutlineColor'] = Color3.fromRGB(0, 0, 0), ["Visible"] = true, ["Text"] = v})
		debugText[v] = Txt
...
writefile('bloxsense/bloxsense_luas/placeholder.lua', "print('lua test'); bloxsense_notify('lua test', 2.5)")
Menu["Settings"]["Configurations"]["Configs"].UpdateValues(getconfigs())
Menu["Settings"]["Scripts"]["Scripts"].UpdateValues(getscripts())
...
do
	--[[
	    ...
	    Lua api documentation:
		 bloxsense_setmenuoptionvalue
	      Info:
		   bloxsense_setmenuoptionvalue({'Tab', 'Section', 'Option', 'Value'}, value)
		  Example:
		    bloxsense_setmenuoptionvalue({"Visuals", "Viewmodel", "Roll", "Value"}, 69)
		 
		 bloxsense_notify
		  Info:
		   bloxsense_notify(message, time)
		  Example:
		   bloxsense_notify('Hello, World!', 5)
	
	]]
	local setmenuoptionvalue = function(path, value)
		local option = Menu
	...
	local notify = function(message, time)
		Library.UI:EventLog(message, time)
	end
	
	env.bloxsense_setmenuoptionvalue = setmenuoptionvalue;		
	env.bloxsense_notify = notify;
end
